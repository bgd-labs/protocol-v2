...
makeSuite('LendingPoolConfigurator', (testEnv: TestEnv) => {
  ...
  it('Deactivates the AAVE reserve for borrowing as ProofOfReserveAdmin', async () => {
    ...
    await addressesProvider.connect(deployer.signer).setAddress(proofOfReserveAdminId, users[2].address);
    ...
    await addressesProvider.connect(deployer.signer).setAddress(proofOfReserveAdminId, deployer.address);
  ...
  it('Disable stable borrow rate on the AAVE reserve as ProofOfReserveAdmin', async () => {
    ...
    const { stableBorrowRateEnabled } = await helpersContract.getReserveConfigurationData(aave.address);
